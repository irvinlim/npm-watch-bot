...
const f = {
    ...
    sendNoSuchPackageMessage: (user, packageName) => {
        return f.sendMessageToUser(user,
            `No such package on npm: ${ packageName }`);
    },

    sendLatestVersionMessage: (user, pkg) => {
        return f.sendMessageToUser(user,
            `Latest version of ${ pkg.get("name") } (https://www.npmjs.com/package/${ pkg.get("name") }) is v${ pkg.get("version") }, `
          + `published ${ moment(pkg.get("date_published")).fromNow() }.`);
    },

    sendNewVersionMessage: (user, pkg) => {
        return f.sendMessageToUser(user,
            `There is a new version of ${ pkg.get("name") } (https://www.npmjs.com/package/${ pkg.get("name") }) available (v${ pkg.get("version") }), `
          + `published ${ moment(pkg.get("date_published")).fromNow() }.` );
    }
};
