...
const koa = new Koa().use(BodyParser());

const rootEndpoint = `${Config.api.endpoint}${Config.telegram.token}/${
  version
}`;

PatchCheckBody(knex, Models).then(() => {
  MountRouters(koa, { bookshelf, knex }, { isNull }, Models); // mount all routers in the ./routers folder
