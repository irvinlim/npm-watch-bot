...
async function prepareDatabase() {
    ...
    if (['dev', 'prod'].some(s => DB_NAME.includes(s))) {
        if (
            await prompt(
                `Are you sure you want to operate on database "${
                    DB_NAME
                }"? (y/n) ` !== 'y'
            )
        ) {
    ...
    if (schemas.length > 0) {
        if (
            (await prompt(
                `Database "${
                    DB_NAME
                }" already exists. Drop the database? (y/n) `
            )) === 'y'
        ) {
    ...
    await tx.schema.createTable(table.name, t => {
        ...
        if (table.timestamps === undefined || table.timestamps === true) {
            ...
            t
                ...
                .notNullable()
                .defaultTo(
                    tx.raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP')
                );
        }
    });
}
    ...
    await tx.schema.table(table.name, t => {
        for (let fromColumn of Object.keys(table.references)) {
            ...
            t
                .foreign(fromColumn)
                .references(toColumn)
                .onUpdate(onUpdateStm)
                .onDelete(onDeleteStm);
        }
    });
}
...
async function getMigrationRows(tx) {
    let migrations = await tx.migrate._listAll({
